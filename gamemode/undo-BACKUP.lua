...
if ( CLIENT ) then
	...
	local function AddUndo( message )
		...
		// Move all the undos up one to make room
		local NewUndo = {}
		for k, v in ipairs( ClientUndos ) do
			NewUndo [ k + 1 ] = v
		end

		NewUndo[ 1 ] = { Key = k, Name = v }
...
function AddFunction( func, ... )
	...
	table.insert( Current_Undo.Functions, { func, arg } )
...
function Do_Undo( undo )
	...
	if ( undo.Entities ) then
		...
		if entity.Team ~= nil && entity.Team ~= undo.Owner:Team() && undo.Owner:Team() ~= 0 then return false end
			if ( entity:IsValid() ) then
			 if entity.Cost ~= nil && undo.Owner:IsValid() then
			     if entity.Built == nil || entity.Built == 2 then
			         if entity.Healthlol == nil || entity.Healthlol >= entity.Maxlol then
			             if entity.health == nil || entity.health >= entity.maxhealth then
			                 if InOutpostRange(undo.Owner, entity:GetPos()) && EnemyMelonCheck(undo.Owner, entity:GetPos()) then 
            			     undo.Owner:SetNetworkedInt("nrg", undo.Owner:GetNetworkedInt("nrg") + (entity.Cost * 0.5))
            			     undo.Owner:PrintMessage(HUD_PRINTTALK, "Sold for " .. (entity.Cost * 0.5) .. " NRG.")
            			     end
            		     end
                    end
                end
