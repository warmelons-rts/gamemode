...
function ScenarioandVariantControl()
	...
	DermaPanel:SetTitle( "WarMelons:RTS Menus (F4)" ) -- Title of the frame
...
local opened = 0--What's this doing here??
xp,yp = 10, 310--What's this doing here??
local EncyclopediaEntry--What's this doing here??
...
function EEPanelCall(Replacer)
	...
	local ini = file.Read("data/WM-RTS/help/picturetemplate.txt","GAME")
	if file.Exists("data/WM-RTS/help/" .. Replacer2 .. ".jpg","GAME") then
		file.Write("data/WM-RTS/picturetemplate.txt", string.gsub(ini, "XXX", Replacer2))
	else
		file.Write("data/WM-RTS/picturetemplate.txt", string.gsub(ini, "XXX", "wmrts"))
	end
    if opened == 1 then
...
function EEPanel(Name, XPos, YPos)
    ...
    if OriginalHelp:GetFloat() == 0 then
		...
		local list = file.Find(tstring,"GAME")
		print("List:")
		PrintTable(list)--DEBUG
		for k, v in pairs(list) do
			fil = fil+1
			if string.find(v, "]" .. Name2) ~= nil then
				print("Found string")--DEBUG
				local date
				if date == nil then 
					print("added a date")--DEBUG
					date = file.Time(tstring,"GAME") 
					FinalLoc = v
					print("FinalLoc:")--DEBUG
					print(FinalLoc)--DEBUG
				end
				print(string.find(v, Name2))--DEBUG
				print(v .. " ")--DEBUG
				tstring = "data/WM-RTS/help/" .. v
				if file.Time(tstring,"GAME") > date then
					date = file.Time(tstring,"GAME")
					FinalLoc = v
					print("FinalLoc:")--DEBUG
					print(FinalLoc)--DEBUG
	...
	print("Files that were checked: "..fil)
	...
	print("FinalLoc:")--DEBUG
	print(FinalLoc)--DEBUG
	if file.Exists("data/WM-RTS/help/" .. FinalLoc,"GAME") then
		html:SetHTML( file.Read("data/WM-RTS/help/" .. FinalLoc,"GAME" ) )
    else
		html:SetHTML( file.Read("data/WM-RTS/help/encyclopediaerror.txt","GAME" ) )
    end
	...
	html2:SetHTML( file.Read("data/WM-RTS/picturetemplate.txt"),"GAME" )
...
function WMCheckServerMaps()
	...
	html5:SetPos( 10, 35 )
	print(file.Exists("data/WM-RTS/wmservermaplist-cur.txt","GAME"))--DEBUG
	if(file.Exists("data/WM-RTS/wmservermaplist-cur.txt","GAME")) then--Added a test to see if file exists.
		html5:SetHTML( file.Read("data/WM-RTS/wmservermaplist-cur.txt","GAME") )
	else
		html5:SetHTML("<p>The file 'data/WM-RTS/wmservermaplist-cur.txt' does not exist!</p>")
	end
end
...
function WMLoadScenario()
	...
	local serversscens = string.Explode("\n<br>\n", file.Read("data/WM-RTS/wmservermaplist-cur.txt","GAME"))
	--PrintTable(serversscens)
    for k, v in pairs (file.Find("data/WM-RTS/scenarios/*.txt", "GAME")) do
        for x, y in pairs (serversscens) do
            if tostring(y) == tostring(v) then
                ...
                function comboitem:DoClick()
					local ini = file.Read("data/WM-RTS/scenarios/" .. self:GetText(),"GAME")
					local exploded = string.Explode("\n", ini);
...
function WMBountyMenu()
	...
	BountySlider:SetMax( LocalPlayer():GetNWInt("nrg") ) -- Maximum number of the slider   
...
function WMInvite()
	...
	local InviteComboBox = vgui.Create( "DComboBox", InviteMenu ) --Creates a DComboBox on the InviteMenu DFrame.
	InviteComboBox:SetPos( 10, 35 )  
	InviteComboBox:SetSize( 105, ScrH()*math.Clamp((0.12+(table.Count(player.GetAll())*0.05 )), 0.1, 0.95) - 75 )  
	--InviteComboBox:SetMultiple( false ) --Disabled because of error.
	...
	InviteButton.DoClick = function()
		if InviteComboBox:GetSelected() and InviteComboBox:GetSelected()[1] then --NEEDS PROPER FIXING!
			...
			JustInvited = true
			--Msg("/n\n")
			for k, v in pairs (InviteComboBox:GetSelected()) do --NEEDS PROPER FIXING!
...
function WMDonate()
	...
	local DonateComboBox = vgui.Create( "DComboBox", DermaPanel )  --Creates a DComboBox on the DermaPanel DFrame.
	...
	--DonateComboBox:SetMultiple( false ) --Disabled because of error.
	for k, v in pairs (player.GetAll()) do
		if v:GetName() ~= LocalPlayer():GetName() then
			DonateComboBox:AddChoice( v:GetName() )
		end
	end
	...
	DonateSlider:SetMax( LocalPlayer():GetNWInt("nrg") ) -- Maximum number of the slider   
	DonateSlider:SetDecimals( 0 ) -- Sets a decimal. Zero means it's an integer  
   
	local DonateButton = vgui.Create("DButton", DermaPanel ) --Creates a DButton on the DermaPanel DFrame.
	...
	DonateButton.DoClick = function()
		if DonateComboBox:GetSelected() and DonateComboBox:GetSelected()[1] then --NEEDS FIXING!
