...
if (CLIENT) then
	language.Add( "Tool_lap_snipers_name", "Sniper War Melons" )
	language.Add( "Tool_lap_snipers_desc", "Long ranged, stealthy melons" )
	language.Add( "Tool_lap_snipers_0", "Left-click to spawn a warmelon. Right-click to display cost." )
	language.Add( "Undone_lap_snipers", "Undone Sniper Warmelon" )
...
function TOOL.BuildCPanel (CPanel)
	CPanel:AddControl ("Header", { Text="#Tool_lap_snipers_name", Description="#Tool_lap_snipers_desc" })
	local VGUI = vgui.Create("WMHelpButton",CPanel);
	VGUI:SetTopic("Sniper");
    	CPanel:AddPanel(VGUI);
	if LocalPlayer():IsAdmin() then
	CPanel:AddControl ("Slider", {
		Label = "Team number (Only applies on Team 0)",
		Command = "lap_snipers_teamnumber",
		Type = "Integer",
		Min = "1",
		Max = "6"
	} )
