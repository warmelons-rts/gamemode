...
if ( SERVER ) then
	...
	function MakeBalloon( pl, r, g, b, force, Data )
		if ( IsValid( pl ) && !pl:CheckLimit( "balloons" ) ) then return nil end
			local cost = math.floor(250 + force * GetConVarNumber( "WM_WheelandThrusterCost", 4 ))
			if !InOutpostRange(pl, Data.Pos)  then return false end
			...
			balloon.Team = pl:Team()
			balloon.Warmelon = true
...
--[[ function MakeBalloon( pl, r, g, b, force, skin, Data ) //!OLD WM:RTS BALOON CODE!
