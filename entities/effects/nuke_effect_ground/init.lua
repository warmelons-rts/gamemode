...
function EFFECT:Init( data )
	...
	self.YieldFast = self.Yield^3
	self.YieldSlow = self.Yield^1.2
	self.YieldSlowest = self.Yield^1
	self.YieldInverse = self.Yield^-1.5
	self.TimeLeft = CurTime() + 5
	self.GAlpha = 255
	self.GSize = 100*self.Yield
	self.FAlpha = 254
	self.CloudHeight = data:GetScale()
--	if self.CloudHeight < 100 then self.CloudHeight = 50 end
	self.Refract = 0.5
	...
	--big firecloud
		for i=1, 150 do
			...
			--particle:VelocityDecay( true )--VelocityDecay REMOVED IN GMOD UPDATE 15.03.09
...
function EFFECT:Think( )
	...
	if timeleft > 0 then 
		if timeleft then
			...
			self.Size = self.Size + 1000*ftime
...
render.DrawSprite(startpos + Vector(0,0,self.CloudHeight),40*self.GSize/2,500*self.GSize/2,Color(130,120,240,0.23*self.GAlpha))
render.DrawSprite(startpos + Vector(0,0,self.CloudHeight),700*self.GSize/2,60*self.GSize/2,Color(80,73,255,self.GAlpha))
